...
function validate_input() {
	...
	// SMALL PHOTO
	$uploadedfilesmall = $_FILES['photo_small'];
	$upload_overrides_small = array( 'test_form' => false );
	
	$movefile_small = wp_handle_upload( $uploadedfilesmall, $upload_overrides_small );
	
	if ( $movefile_small && ! isset( $movefile_small['error'] ) ) {
		// echo "File is valid, and was successfully uploaded.\n";
		if (in_array($movefile_small['type'], array('image/jpeg', 'image/png'))) {
			$img = wp_get_image_editor( $movefile_small['file'] );
			if ( ! is_wp_error( $img ) ) {
				$img->save($movefile_small['file']);
				$_POST['photo_small_text'] = substr($movefile_small['file'], strlen(ABSPATH));
			}
		} else {
			$result[] = "Photo (small) image must be in jpeg and png format!";
		}
	} else {
		/**
		 * Error generated by _wp_handle_upload()
		 * @see _wp_handle_upload() in wp-admin/includes/file.php
		 */
		if ($uploadedfilesmall['size'] !== 0) {
			$result[] = $movefile_small['error'];
		} else {
			$_POST['no_small_image'] = true;
		}
	}
	
	$_POST['photo_small'] = $_POST['old_photo_small'];
	
	if (count($result) > 0) {
		unlink($movefile['file']);
		unlink($movefile_small['file']);
	} else {
		if (empty($_POST['photo_small_text'])) {
			$_POST['photo_small_text'] = $_POST['photo_small'];
		}
		if (!$_POST['no_small_image']) {
			unlink(ABSPATH . $_POST['old_photo_small']);
			$_POST['photo_small'] = substr($movefile_small['file'], strlen(ABSPATH));
		}
	}
	...
	if (count($errors) === 0) {
		$result = $wpdb->update(
				...
				array(
						...
						'photo_small'  => $_POST['photo_small_text']
				), //data
				array('discipline_id' => $_POST['discipline_id']),
				array('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%s', '%s'), //data format
				array("%d")
		);
	}
...
<div class="wrap">
	...
	<form method="post" name="updatediscipline" id="updatediscipline" class="validate" enctype="multipart/form-data"
		...
		<table class="form-table">
			<tbody>
				...
				<tr class="form-field">
					<th scope="row"><label for="photo">Photo <span class="description">1200px x 300px</span></label></th>
				...
				<tr class="form-field">
					<th scope="row"><label for="photo_small">Small Photo <span class="description">355px x 275px</span></label></th>
					<td><input type="hidden" name="old_photo_small" value="<?= $data['photo_small'] ?>">
						<img id="photo_img_small" src="<?=  empty($data['photo_small']) ? "#" : (home_url( '/' ) . $data['photo_small']) ?>" />
						Choose other <input type="file" name="photo_small" id="photo_small">
					</td>
				</tr>
			</tbody>
		</table>
...
jQuery("#photo").change(function(){
    readURL('photo_img', this);
});

jQuery("#photo_small").change(function(){
    readURL('photo_img_small', this);
});
</script>
