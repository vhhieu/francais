...
class Profs_List_Table extends WP_List_Table {
	...
	function get_columns() {
		return array(
				...
				"city_list" => "Ville",
				"micro_discipline_list" => "Micro Discipline"
		);
	}
	 ...
	 **************************************************************************/
	function column_profs_index($item){
	
		//Build row actions
		$actions = array(
				'edit'      => sprintf('<a href="?page=francais-profs-edit&movie=%s">Edit</a>', $item['profs_id']),
				'delete'    => sprintf('<a href="?page=%s&action=%s&movie=%s">Delete</a>',$_REQUEST['page'],'delete',$item['profs_id']),
		);
	
		$value = $item['first_name'] . " " . $item['family_name'];
	...
	function column_cb($item){
		return sprintf(
				...
				/*$2%s*/ $item['profs_id']                //The value of the checkbox should be the record's id
				);
	}
	...
	function process_bulk_action() {
		...
		if( 'delete'===$this->current_action() && isset($_REQUEST['movie'])) {
			...
			$result = "";
			if (!empty($ids)) {
				$result = $wpdb->query("DELETE FROM " . $wpdb->prefix . "francais_profs WHERE profs_id IN($ids)");
			}
...
$testListTable = new Profs_List_Table();
